...
export default function ClubPage() {
  ...
  return (
    <AppShell>
      ...
      {selected && <div role="dialog" aria-modal="true" aria-labelledby="coupon-title" className="fixed inset-0 z-[80] grid place-items-center bg-[#08160f]/70 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedId(''); }}><section className="w-full max-w-md rounded-[28px] bg-[#fbf9f4] p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between"><span className="grid size-12 place-items-center rounded-full bg-[#e4eee7] text-[#18563b]"><TicketCheck className="size-5" /></span><button type="button" onClick={() => setSelectedId('')} aria-label="Fechar cupom" className="grid size-10 place-items-center rounded-full hover:bg-[#eee9df]"><X className="size-5" /></button></div><p className="mt-6 text-xs font-bold uppercase tracking-[0.16em] text-[#92743b]">Cupom Clube Gante</p><h2 id="coupon-title" className="mt-2 font-serif text-3xl">{selected.partner}</h2><p className="mt-2 text-sm text-[#70746e]">{selected.offer} · {selected.detail}</p><div className="mt-6 rounded-2xl border-2 border-dashed border-[#bda667] bg-[#f1e8cf] p-5 text-center"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#79642f]">Seu código</p><p className="mt-2 font-mono text-xl font-bold tracking-[0.08em] text-[#173b2b]">{coupon}</p></div><button type="button" onClick={() => void copyCoupon()} className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#174c35] text-sm font-bold text-white">{copied ? <><Check className="size-4" /> Código copiado</> : <><Copy className="size-4" /> Copiar cupom</>}</button><p className="mt-4 text-center text-xs leading-5 text-[#777b75]">Apresente este código ao parceiro. A disponibilidade e as condições são confirmadas no local.</p></section></div>}
    </AppShell>
  );
}
